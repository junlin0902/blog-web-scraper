...
export const lookAtServiceFile = async (file, context) => {
    const { gql, prisma, pathSettings: settings, codeFacts: serviceFacts, fieldFacts } = context;
    if (!gql)
        throw new Error(`No schema when wanting to look at service file: ${file}`);
    if (!prisma)
        throw new Error(`No prisma schema when wanting to look at service file: ${file}`);
    ...
    if (!mutationType)
        throw new Error("No mutation type");
    ...
    if (Object.keys(fileFacts).length === 0)
        return;
    // Tracks prospective prisma models which are used in the file
    const extraPrismaReferences = new Set();
    const extraSharedFileImportReferences = new Set();
    ...
    const knownSpecialCasesForGraphQL = new Set();
    ...
    Object.values(fileFacts).forEach((model) => {
        ...
        if (skip.includes(model.typeName))
            return;
    ...
    if (aliases.length) {
        fileDTS.addTypeAliases(aliases.map((s) => ({
            name: s,
            type: "any",
        })));
    }
    ...
    return dtsFilepath;
    function addDefinitionsForTopLevelResolvers(parentName, config) {
        const { name } = config;
        ...
        if (parentName === queryType.name)
            knownSpecialCasesForGraphQL.add(queryType.name);
        if (parentName === mutationType.name)
            knownSpecialCasesForGraphQL.add(mutationType.name);
    ...
    function addCustomTypeModel(modelFacts) {
        ...
        if (!parentIsPrisma)
            extraSharedFileImportReferences.add({ name: `S${modelName}`, import: modelName });
        ...
        modelFacts.resolvers.forEach((resolver) => {
            ...
            if (field) {
                ...
                if (modelFieldFacts[fieldName])
                    modelFieldFacts[fieldName].hasResolverImplementation = true;
                else
                    modelFieldFacts[fieldName] = { hasResolverImplementation: true };
        ...
        function createParentAdditionallyDefinedFunctions() {
            ...
            modelFacts.resolvers.forEach((resolver) => {
                ...
                if (!existsInGraphQLSchema) {
                    console.warn(`The service file ${filename} has a field ${resolver.name} on ${modelName} that does not exist in the generated schema.graphql`);
                }
...
function returnTypeForResolver(mapper, field, resolver) {
    if (!field)
        return "void";
    ...
    if (resolver.isFunc && resolver.isAsync)
        returnType = `Promise<${tType}>`;
    else if (resolver.isFunc && resolver.isObjLiteral)
        returnType = tType;
    else if (resolver.isFunc)
        returnType = all;
    else if (resolver.isObjLiteral)
        returnType = tType;
    else if (resolver.isUnknown)
        returnType = all;
    return returnType;
}
