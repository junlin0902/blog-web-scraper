...
export const createSharedSchemaFiles = async (context) => {
...
async function createSharedExternalSchemaFile(context) {
    ...
    if (scalars.length) {
        externalTSFile.addTypeAliases(scalars.map((s) => ({
            name: s,
            type: "any",
        })));
    }
...
async function createSharedReturnPositionSchemaFile(context) {
    const { gql, prisma, fieldFacts } = context;
    const types = gql.getTypeMap();
    const mapper = typeMapper(context, { preferPrismaModels: true });
    const typesToImport = [];
    const knownPrimitives = ["String", "Boolean", "Int"];
    const externalTSFile = context.tsProject.createSourceFile(`/source/${context.pathSettings.sharedInternalFilename}`, `
