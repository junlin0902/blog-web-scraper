            ...
            for (const type of Object.values(schema.getTypeMap())) {
                if ((!opts.skipIntrospection || !isIntrospectionType(type)) && isObjectType(type)) {
                    for (const field of Object.values(type.getFields())) {
                        ...
                        field.resolve = (root, args, context, info) => mapMaybePromise(onResolve({
                            root,
                            args,
                            context,
                            info,
                            ...
                            if (typeof afterResolve === 'function') {
                                try {
                                    return mapMaybePromise(resolver(root, args, context, info), result => mapMaybePromise(afterResolve({
                                        result,
                                        setResult: newResult => {
                                            result = newResult;
                                        },
                                    }), () => result), errorResult => mapMaybePromise(afterResolve({
                                        result: errorResult,
                                        setResult: newResult => {
                                    ...
                                    let errorResult = err;
                                    return mapMaybePromise(afterResolve({
                                        result: errorResult,
                                        setResult: newResult => {
                                            errorResult = newResult;
                                        },
